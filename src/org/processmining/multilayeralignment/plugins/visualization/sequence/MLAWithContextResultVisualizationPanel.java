...
public class MLAWithContextResultVisualizationPanel extends JPanel implements ActionListener {
...
//	protected ProMPropertiesPanel logAlignmentPanel;
	...
	public MLAWithContextResultVisualizationPanel(MLAWithContextReplayResult replayResult,XLog inProcessLog,XLog inDataLog, Progress progress) {
	...
	private Component createBottomPanel() {
		...
		String link [] = {"Projection to process log"};
	...
	protected void addMultiLayerAlignments(){
		for (int i = (selectedPage - 1) * numOfalignmentsPerPage; i < Math.min(selectedPage * numOfalignmentsPerPage, replayResult.size()); i++) {
			...
			MLAWithContextInstanceConformanceView alignmentPanel = createMultiLayerInstanceView(traceIndex,nodeInstance,stepTypes,nodeInstanceVOL,stepTypesVOL,rearrangedNodeInstance,processTrace,dataTrace);
	...
	protected MLAWithContextInstanceConformanceView createMultiLayerInstanceView(String traceIndex,List<Object> nodeInstance, List<StepTypes> stepTypes, List<Object> nodeInstanceVOL, List<StepTypesVOL> stepTypesVOL,List<Object> rearrangedNodeInstances,XTrace processTrace,XTrace dataTrace) {
		return new MLAWithContextInstanceConformanceView(traceIndex,nodeInstance,stepTypes,nodeInstanceVOL,stepTypesVOL,rearrangedNodeInstances,40,processTrace,dataTrace);
	}
	...
	protected Component createLegendPanel() {
        ...
        //pattern1:TSM: expected Activity and Data operation
		layout.insertRow(row, TableLayout.PREFERRED);
		...
		JLabel TSM = factory.createLabel(" - Expected activity and data operations");
		...
		JLabel TSMP = factory.createLabel(" - Expected activity and data operations were done by an illegitimate role");
		...
		JLabel PSM = factory.createLabel(" - Expected activity with all data operations ignored ");
		...
		JLabel PSMP = factory.createLabel(" - Expected activity with all data operations ignored was perfomed by an illegitimate role");
		...
		JPanel grayPanel = new JPanel(){
			protected void paintComponent(Graphics g) {
				...
				//Data Layer
				y+= 40;
				int[] xCoords2 = new int[] { x, x + width / 2 , x + width / 2, };
				int[] yCoords2 = new int[] { y + height / 2, y, y + (height / 2)};
		...
		grayPanel.setPreferredSize(new Dimension(iconSize, 90));
		grayPanel.setSize(new Dimension(iconSize, 90));
		legendPanel.add(grayPanel, "0," + row + ",r, c");
		JLabel TSMPSM = factory.createLabel(" - Expected activity with some ignored data operations");
		legendPanel.add(TSMPSM, "1," + row++ + ",l, c");
		...
		blackPanel.setPreferredSize(new Dimension(iconSize, 90));
		blackPanel.setSize(new Dimension(iconSize, 90));
		legendPanel.add(blackPanel, "0," + row + ",r, c");
		JLabel TSMPPSMP = factory.createLabel(" - Expected activity with some ignored data operations was performed by an illegitimate role");
		legendPanel.add(TSMPPSMP, "1," + row++ + ",l, c");
		...
		JLabel MM = factory.createLabel(" - Missing activity and data operations");
		...
		JLabel PLM = factory.createLabel(" -Unexpected activity");
		...
		JPanel redPanel = new JPanel(){
			protected void paintComponent(Graphics g) {
				...
				//privacy layer
				g2d.setColor(Color.red);
				g2d.fillOval(x + 8 , y, 16, 16);
				g2d.setColor(Color.black);
				g2d.drawOval(x + 8 , y, 16, 16);
				int y1=y+17;
		...
		JLabel DLM = factory.createLabel(" - Unexpedted data operation");
