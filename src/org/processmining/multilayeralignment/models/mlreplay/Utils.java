...
public class Utils {
	...
	public enum Statistic {
		...
		PMCOST("PSynchronous move cost of alignment"), //
		DMCOST("Data Log move cost of alignment"),//
	...
	private static void transitionToDot(SyncProduct product, PrintStream stream, int i, int t) {
		...
		stream.print("<br/>cost:" + product.getCost(t));
		stream.print(">");

		if (product.getTypeOf(t) == SyncProduct.LOG_MOVE) {
			stream.print(",style=filled,fillcolor=Salmon,fontcolor=black");
		} else if (product.getTypeOf(t) == SyncProduct.MODEL_MOVE) {
			stream.print(",style=filled,fillcolor=blue,fontcolor=white");
			...
			stream.print(",style=filled,fillcolor=yellow,fontcolor=Black");
		}else if (product.getTypeOf(t) == SyncProduct.DLOG_MOVE) {
			stream.print(",style=filled,fillcolor=red,fontcolor=white");
		}else if (product.getTypeOf(t) == SyncProduct.PSYNC_MOVE_T2) {
			stream.print(",style=filled,fillcolor=MediumSlateBlue,fontcolor=white");
		}else if (product.getTypeOf(t) == SyncProduct.OPSYNC_MOVE_T2) {
			stream.print(",style=filled,fillcolor=darkyellow,fontcolor=Black");
		}else if (product.getTypeOf(t) == SyncProduct.SYNC_MOVE_T2) {
			stream.print(",style=filled,fillcolor=orange,fontcolor=white");
		}
   ...
   //rewrite this
	public static TObjectIntMap<XEventClass> createClass2IDDataEvent(XEventClasses classes) {
	...
	public static TObjectIntMap<XEventClass> createClass2IDProcessEvent(Map<XEventClass, Integer> classes) {
		TObjectIntHashMap<XEventClass> c2id = new TObjectIntHashMap<>(classes.size(), 0.75f, -1);
		int id = 0;
		for (Map.Entry<XEventClass, Integer> entry : classes.entrySet()) {
