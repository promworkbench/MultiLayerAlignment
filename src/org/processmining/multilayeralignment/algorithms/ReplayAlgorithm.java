package org.processmining.multilayeralignment.algorithms;
			...
			public void writeEdgeTraversed(ReplayAlgorithm algorithm, int fromMarking, int transition, int toMarking,
				...
				if (transition >= 0) {
					...
					if (algorithm.getNet().getTypeOf(transition) == SyncProduct.SYNC_MOVE) {
						b.append(",style=filled,fillcolor=forestgreen,fontcolor=white");
					} else if (algorithm.getNet().getTypeOf(transition) == SyncProduct.MODEL_MOVE) {
						b.append(",style=filled,fillcolor=darkorchid1,fontcolor=white");
					} else if (algorithm.getNet().getTypeOf(transition) == SyncProduct.LOG_MOVE) {
						b.append(",style=filled,fillcolor=goldenrod2,fontcolor=black");
					} else if (algorithm.getNet().getTypeOf(transition) == SyncProduct.TAU_MOVE) {
						b.append(",style=filled,fillcolor=honeydew4,fontcolor=white");
					}else if (algorithm.getNet().getTypeOf(transition) == SyncProduct.PSYNC_MOVE) {
						b.append(",style=filled,fillcolor=blue,fontcolor=white");
					}else if (algorithm.getNet().getTypeOf(transition) == SyncProduct.DLOG_MOVE) {
						b.append(",style=filled,fillcolor=red,fontcolor=white");
					}
