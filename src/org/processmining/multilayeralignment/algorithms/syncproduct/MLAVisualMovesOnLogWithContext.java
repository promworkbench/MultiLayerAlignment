...
public class MLAVisualMovesOnLogWithContext {
	...
	public MLAVisualMovesOnLogWithContext(SyncReplayResult res) {
		...
		for (int step=0; step < res.getNodeInstance().size(); step++) {
			...
			if(type.toString().equalsIgnoreCase("PSM") || type.toString().equalsIgnoreCase("PSMP")){
				...
				String matchedPart= node.toString().substring(node.toString().indexOf("@")+1,node.toString().length());
				processEventIndex = matchedPart.substring(matchedPart.indexOf(",")+3,matchedPart.length());
				operation="N";
				operationType="N";
				moveType=type.toString();
				
				originalAlignmentLable=matchedProcessEventID+"|"+activity+"|"+operation+"#"+activitySCType+"|"+processEventIndex;
           ...
           if(type.toString().equalsIgnoreCase("TSM") || type.toString().equalsIgnoreCase("TSMP")){
        	   String dataEventIndex="";
				...
				String matchedPart= node.toString().substring(node.toString().indexOf("@")+1,node.toString().length());
				dataEventIndex = matchedPart.substring(matchedPart.indexOf(",")+3,matchedPart.length());
				activitySCType="N";
				...
				originalAlignmentLable=matchedProcessEventID+"|"+activity+"|"+operation+"#"+activitySCType+"|"+dataEventIndex;
				stepTypesOriginalAlignment.add(res.getStepTypes().get(step));
				nodeInstanceOriginalAlignment.add(originalAlignmentLable);
       	   ...
       	   if(type.toString().equalsIgnoreCase("MM")) {
       	    ...
       	    //Tau Moves  
		   /*
       	   if(type.toString().equalsIgnoreCase("TM")){
			
			originalModelTransitionId = node.toString();
		    activity="";
		    matchedProcessEventID= "";
		    activitySCType="";
		    operation="";
		    operationType="";
		    moveType=type.toString();
		    stepTypesOriginalAlignment.add(res.getStepTypes().get(step));
			nodeInstanceOriginalAlignment.add(node.toString());
			value1="@ac:"+moveType +"#"+matchedProcessEventID+"|"+activity;
			...
			*/
			...
			if(type.toString().equalsIgnoreCase("PLM")){
    		    ...
    		    operationType="N";
    		    moveType=type.toString();
    		    processEventIndex=node.toString().substring(node.toString().indexOf("(")+1, node.toString().indexOf(")"));
			...
			if(type.toString().equalsIgnoreCase("DLM")){
    		    ...
    		    operation=node.toString().substring(ordinalIndexOf(node.toString(), ":", 1)+1,node.toString().indexOf("@"))+"|"+dataEventID;
